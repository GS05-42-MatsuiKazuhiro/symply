...
$yourlevel  = $_POST["yourlevel"];
$place  = $_POST["place"];

//2. DB接続します
try {
  $pdo = new PDO('mysql:dbname=symply_db;charset=utf8;host=localhost','root','');
} catch (PDOException $e) {
  exit('DbConnectError:'.$e->getMessage());
}

//３．データ登録SQL作成
$stmt = $pdo->prepare("INSERT INTO tennis(id, lastname, firstname, sex, age, college, email, email2, mylevel, yourlevel, place, indate )VALUES(NULL, :a1, :a2, :a3, :a4, :a5, :a6, :a7, :a8, :a9, :a10, sysdate())");
...
$stmt->bindValue(':a8', $mylevel, PDO::PARAM_INT);
$stmt->bindValue(':a9', $yourlevel, PDO::PARAM_INT);
$stmt->bindValue(':a10', $place, PDO::PARAM_INT);
$status = $stmt->execute();
...
if (isset($_POST['lastname'])){
    ...
    if ($name===""){
      ...
      $isError = true;
        ...
        echo'<a href="tennis.php"><input type="submit" value="戻る"><br><br></a>';
    }
  } else {
    // 未設定のときエラー
    $isError = true;
...
<!-- エラーがなかったとき -->
  ...
  <form method="POST" action="dating.html">
